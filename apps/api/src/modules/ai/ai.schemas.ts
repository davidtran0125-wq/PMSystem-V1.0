...
const severity = z
  ...
  .describe('Mức độ nghiêm trọng: info = ghi nhận, warning = nên xử lý, critical = phải xử lý trước khi duyệt');

export const PurchaseRequestAnalysisSchema = z.object({
  summary: z
    .string()
    .describe('Tóm tắt yêu cầu trong 1-2 câu tiếng Việt, nêu rõ mua gì và để làm gì'),
  completenessScore: z
    .number()
    .int()
    .min(0)
    .max(100)
    .describe('Điểm đầy đủ thông tin 0-100. Dưới 60 nghĩa là thiếu thông tin quan trọng để đi tiếp'),
  readyForRfq: z
    .boolean()
  ...
  missingInformation: z
    ...
    .describe('Danh sách thông tin còn thiếu hoặc chưa rõ. Để mảng rỗng nếu đã đầy đủ'),
  risks: z
    .array(
      z.object({
    ...
    .describe('Rủi ro về kỹ thuật, ngân sách, tiến độ, tuân thủ. Để mảng rỗng nếu không có'),
  suggestedQuestions: z
    .array(z.string())
    .describe('Câu hỏi cụ thể buyer nên hỏi lại người yêu cầu. Tối đa 5 câu'),
  budgetAssessment: z
    .string()
    .describe('Nhận xét về ngân sách và đơn giá dự kiến so với mặt bằng thị trường. Nếu không đủ dữ liệu, nói rõ là không đủ căn cứ'),
});
...
export const SupplierSuggestionSchema = z.object({
  ...
  suggestions: z
    .array(
      z.object({
        supplierId: z.string().describe('id của nhà cung cấp, lấy đúng từ danh sách được cung cấp'),
        ...
        concerns: z.array(z.string()).describe('Điểm cần lưu ý. Để mảng rỗng nếu không có'),
        recommendation: z
          .enum(['mời_ngay', 'cân_nhắc', 'không_phù_hợp'])
          .describe('Khuyến nghị có nên mời báo giá không'),
      }),
    )
    .describe('Xếp hạng giảm dần theo fitScore. Chỉ dùng nhà cung cấp có trong danh sách đầu vào'),
});
...
export const QuotationAnalysisSchema = z.object({
  ...
  recommendationReason: z
    .string()
    .describe('Lý do khuyến nghị, cân nhắc cả giá lẫn các yếu tố khác chứ không chỉ giá thấp nhất'),
  totalCostOfOwnership: z
    .string()
    .describe('Nhận xét về tổng chi phí sở hữu: bảo hành, thời gian giao, điều khoản thanh toán ảnh hưởng thế nào ngoài đơn giá'),
  comparison: z
    .array(
      z.object({
        quotationId: z.string().describe('id báo giá, lấy đúng từ dữ liệu đầu vào'),
  ...
  redFlags: z
    .array(
      z.object({
        quotationId: z.string().nullable().describe('id báo giá liên quan, null nếu là vấn đề chung'),
        issue: z.string().describe('Dấu hiệu bất thường'),
        severity,
      }),
    )
    .describe('Bất thường như giá lệch quá xa mặt bằng, điều khoản mập mờ, hiệu lực quá ngắn. Để mảng rỗng nếu không có'),
  negotiationPoints: z
    .array(z.string())
...
export const ContractReviewSchema = z.object({
  ...
  missingClauses: z
    ...
    .describe('Điều khoản tiêu chuẩn nên có nhưng chưa thấy: phạt chậm giao, bảo hành, chấm dứt, bảo mật, bất khả kháng...'),
  keyDates: z
    .array(
      z.object({
        label: z.string().describe('Tên mốc thời gian'),
        date: z.string().describe('Ngày, định dạng YYYY-MM-DD nếu xác định được'),
...
export const QuotationExtractionSchema = z.object({
  supplierName: z.string().nullable().describe('Tên nhà cung cấp trên chứng từ'),
  quotationNumber: z.string().nullable().describe('Số báo giá'),
  quotationDate: z.string().nullable().describe('Ngày báo giá, định dạng YYYY-MM-DD'),
  currency: z.string().nullable().describe('Mã tiền tệ, ví dụ VND hoặc USD'),
  paymentTerm: z.string().nullable(),
  incoterm: z.string().nullable(),
  leadTimeDays: z.number().int().nullable().describe('Thời gian giao hàng quy đổi ra số ngày'),
  warranty: z.string().nullable(),
  validUntil: z.string().nullable().describe('Hiệu lực báo giá đến ngày, định dạng YYYY-MM-DD'),
  items: z
    .array(
      z.object({
  ...
  confidence: z
    ...
    .min(0)
    .max(100)
    .describe('Mức độ tự tin vào kết quả đọc 0-100. Dưới 70 nghĩa là người dùng nên kiểm tra lại'),
  warnings: z
    .array(z.string())
    .describe('Chỗ đọc không chắc chắn hoặc thiếu trên chứng từ. Để mảng rỗng nếu đọc rõ ràng'),
});

export type PurchaseRequestAnalysis = z.infer<typeof PurchaseRequestAnalysisSchema>;
